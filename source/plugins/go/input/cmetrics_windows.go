...
/*
#cgo LDFLAGS: -LC:/cmetrics/lib -lcmetrics -lcfl -lmpack -lxxhash
#cgo CFLAGS: -IC:/cmetrics/include -w
...
func (g *Gauge) Add(ts time.Time, value float64, labels []string) error {
	ret := C.cmt_gauge_add(g.gauge, C.size_t(ts.UnixNano()), C.double(value), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Gauge) Inc(ts time.Time, labels []string) error {
	ret := C.cmt_gauge_inc(g.gauge, C.size_t(ts.UnixNano()), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Gauge) Dec(ts time.Time, labels []string) error {
	ret := C.cmt_gauge_dec(g.gauge, C.size_t(ts.UnixNano()), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Gauge) Sub(ts time.Time, value float64, labels []string) error {
	ret := C.cmt_gauge_sub(g.gauge, C.size_t(ts.UnixNano()), C.double(value), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Gauge) Set(ts time.Time, value float64, labels []string) error {
	ret := C.cmt_gauge_set(g.gauge, C.size_t(ts.UnixNano()), C.double(value), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func NewContextSetFromMsgPack(msgPackBuffer []byte, offset int) ([]*Context, error) {
	...
	for ret == C.CMT_DECODE_MSGPACK_SUCCESS {
		...
		ret = C.cmt_decode_msgpack_create(&ct.context, cBuffer, C.size_t(len(msgPackBuffer)), (*C.size_t)(unsafe.Pointer(&offset)))
...
func NewContextFromMsgPack(msgPackBuffer []byte, offset int64) (*Context, error) {
	...
	cOffset = (*C.size_t)(unsafe.Pointer(&offset))
	ret := C.cmt_decode_msgpack_create(&ct.context, cBuffer, C.size_t(len(msgPackBuffer)), cOffset)
	if ret != 0 {
...
func (g *Counter) Add(ts time.Time, value float64, labels []string) error {
	ret := C.cmt_counter_add(g.counter, C.size_t(ts.UnixNano()), C.double(value), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Counter) Inc(ts time.Time, labels []string) error {
	ret := C.cmt_counter_inc(g.counter, C.size_t(ts.UnixNano()), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
...
func (g *Counter) Set(ts time.Time, value float64, labels []string) error {
	ret := C.cmt_counter_set(g.counter, C.size_t(ts.UnixNano()), C.double(value), C.int(len(labels)), GoStringArrayToCptr(labels))
	if ret != 0 {
